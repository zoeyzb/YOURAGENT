...
function actionConfig(action: ReturnType<typeof requestedHttpActions>[number]) {
  return {
    ...
    ...(action.bodyTemplate !== undefined && action.bodyTemplate !== null ? { bodyTemplate: action.bodyTemplate } : {}),
  };
}
...
function updatePreservedWorkflow(previous: AgentConfig, payload: AgentBuilderInput): Workflow {
  ...
  for (const [index, action] of actions.entries()) {
    ...
    if (existing) {
      ...
      if (action.bodyTemplate === null) delete existing.config.bodyTemplate;
      continue;
    }
